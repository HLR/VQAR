...
def program_declaration(data_list):
    ...
    from regr.program import POIProgram, IMLProgram, SolverPOIProgram, SolverListPOIProgram
    ...
    ###### chen: SolverListPOIProgram
    list_poi = [[sentence[entailment], sentence[neutral], sentence[contradiction]]]*len(data_list)
    program = SolverListPOIProgram(graph, inferTypes=['ILP', 'local/argmax'], list_poi=list_poi,
                            loss=MacroAverageTracker(NBCrossEntropyLoss()),
                            metric={'ILP': PRF1Tracker(DatanodeCMMetric()),
                                    'argmax': PRF1Tracker(DatanodeCMMetric('local/argmax'))})
...
def main(args):
    ...
    model = program_declaration(train_dataset)
    ...
    ### chen repeat here. 
    model = program_declaration(test_dataset)
    ### chen repeat here
