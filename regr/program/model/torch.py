...
class TorchModel(BaseModel):
   ...
   ### chen: add an optinal variable: use_list_poi = True/False
    def __init__(self, graph, loss=None, metric=None, device='auto',use_list_poi=False ):
    ...
    ### chen: add find_list_poi function. need to discuss with hossein here.
    ### what is the meaning of two for loops.
    def find_list_poi(self):
        for prop in self.graph.traversal_apply(all_properties):
            for sensor1, sensor2 in combinations(prop.find(self.BaseSensor), r=2):
                if sensor1.target:
                    target_sensor = sensor1
...
class PoiModel(TorchModel):
    ...
    ### chen: add an optinal variable: use_list_poi = True/False
    def forward(self, data_item, inference=True, use_list_poi=False):
