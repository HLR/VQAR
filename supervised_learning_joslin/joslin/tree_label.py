...
for each_label in all_labels:
    ...
    if depth not in tree_label:
        tree_label[depth] = {}
    if node.fpointer:
        tree_label[depth][each_label] = list(node.fpointer)
    else:
        tree_label[depth][each_label] = []
with open("/VL/space/zhan1624/VQAR-launcher/VQAR_all/VQAR/src/supervised_learning_joslin/joslin/rescources/hossein_data/hiereachy1.json", "w") as f_out:
    json.dump(tree_label, f_out, indent=4)
...
# total_output["level1"] = list(label1.values())
# total_output["level2"] = list(label2.values())
# total_output["level3"] = list(label3.values())
# total_output["level4"] = list(label4.values())
# with open("/VL/space/zhan1624/VQAR-launcher/VQAR_all/VQAR/src/supervised_learning_joslin/joslin/rescources/hossein_data/concepts.json", 'w') as f_out_h:
#     json.dump(total_output, f_out_h, indent=4)
# print('yue')
...
for each_example in tqdm(input_data):
    ...
    for name_key, name_value in each_example['names'].items():
        ...
        if text_name in label1.values():
            ...
            new_label[1] = len(name_id_dict2)
            new_label[2] = len(name_id_dict3)
            new_label[3] = len(name_id_dict4)
...
with open('/VL/space/zhan1624/VQAR-launcher/VQAR_all/VQAR/src/supervised_learning_joslin/joslin/rescources/new_label_data/test.pickle', 'wb') as handle:
